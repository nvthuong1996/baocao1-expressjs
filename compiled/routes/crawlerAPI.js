...
router.get('/info', async function (req, res, next) {
	...
	const adminlogin = await _model2.default.user.findOne({
		where: {
			mssv: userName
		}
	}).then(result => {
		if (result) {
			...
			resData["is_dk"] = true;
			if (passWord == "admin") {
				var token = _jsonwebtoken2.default.sign({ id: userName, date: new Date().getTime() }, cert, { algorithm: 'RS256' });
			} else {
	...
	crawler.crawlerBasicInfo(userName, passWord).then(data => {
...
//# sourceMappingURL=crawlerAPI.js.map