...
router.get("/", async (req, res, next) => {
	...
	const conversion = await _controller.conversionController.loadConversionType1(user);
...
router.get("/group", async (req, res, next) => {
	const user = req.user.mssv;
	const conversion = await _controller.conversionController.loadConversionType2(user);
	return res.render('conversion', {
		conversions: conversion,
		type: 2
	});
...
router.get("/:id", async (req, res, next) => {
	...
	const user = req.user.mssv;
	const conversion = await _model2.default.user_conversion.findOne({
		where: {
			user_id: user,
...
router.post("/send", async (req, res, next) => {
	...
	try {
		if (nguoigui == nguoinhan) throw new Error(`Bạn không thể gửi tin nhắn cho chính mình`);

		const user = await _model2.default.user.findOne({
			where: {
				mssv: nguoinhan
			}
		});
		...
		const conversion_nguoigui = await _model2.default.user_conversion.findAll({
			where: {
				user_id: nguoigui
			}
		});

		const conversion_nguoinhan = await _model2.default.user_conversion.findAll({
			where: {
				user_id: nguoinhan
			}
		});
		...
		const createMessage = await _model2.default.message.create({
			creator_id: nguoigui,
			message_body: JSON.stringify(m),
			conversion_id: conversion_id
		});

		(0, _sockets.sendMessageToRoom)(nguoigui, conversion_id, m);
	...
	if (conversion_id) {
		...
		return conversion_id.reduce((start, item) => {
			return start.then(result => {
				...
				return _model2.default.conversion.findOne({
					where: {
						id: item
					}
				}).then(result => {
		...
		return _model2.default.conversion.create({
			name: null,
			is_active: 1,
			type: 1
		}).then(createConversion => {
			return _model2.default.user_conversion.create({
...
async function getMessage(conversion_id, user) {
	...
	const conversion = await _model2.default.conversion.findOne({
		where: {
			id: conversion_id
		}
	});

	const _messages = await _model2.default.message.findAll({
		where: {
			conversion_id: conversion_id
		},
	...
	if (conversion.type == 1) {
		...
		const user_conversion = await _model2.default.user_conversion.findAll({
			where: {
				conversion_id: conversion_id
			}
		});
...
function getFullInfoUser(id) {
	return _model2.default.user.findOne({
		...
		return _model2.default.fb_account.findOne({
			where: {
